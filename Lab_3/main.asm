...
Summation	mov.w 	R5, R6		;starting the upper limit with value a of the summation in R6
			...
			rla.w	R6			;finish Xcalcby dobling the value and adding it to accumulator R8,
			add.w	R6, R8
			dec.w	R5
			jnz		Summation  ;repeat the loop with a-1, a-2 (in R5) till lower value a=0
...
FACTO:		tst		R5			;write the factorial subroutine
			jz		FACTOZERO
			push.w	R5

			mov.w	R5,R6		;you start with value n in R6, then multiply with n-1, n-2, ..1
LOOP		dec.w	R5
			jz		END
			call	#MULT  ;use the MULT subroutine to multiply R5 to R6 registers
			jmp		LOOP
